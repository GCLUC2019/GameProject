...
{
	m_ui_pos = CVector2D(780, 670);
	m_ui_size = CVector2D(500, 50);

	m_hp_frame_image_p = GET_RESOURCE("Boss_HP_Bar_Frame", CImage*);
	m_hp_bar_p = new CBar(GET_RESOURCE("Boss_HP_Bar", CImage*), CCharacterBoss::GetInstance()->GetHitPointPointer(), CCharacterBoss::GetInstance()->GetHitPointMax(), m_ui_pos + CVector2D(9, 6.0), CVector2D(481,38));
