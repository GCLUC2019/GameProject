...
CGameSceneUI::CGameSceneUI() :CObject(0, DP_UI)
{
	...
	m_hp_bar_p = new CBar(GET_RESOURCE("HP_Bar", CImage*), CCharacterPlayer::GetInstance()->GetHitPointPointer(), CCharacterPlayer::GetInstance()->GetHitPointMax(), m_ui_pos + CVector2D(160, 137) * 0.8, CVector2D(330, 25) * 0.8);

	m_weapon_bar_p = new CBar(GET_RESOURCE("Weapon_Bar", CImage*), CCharacterPlayer::GetInstance()->GetPlayerEquipWeaponEndurancePointer(), ENDURANCE_MAX, m_ui_pos + CVector2D(196, 183), CVector2D(330*0.6, 25*0.6));
...
void CGameSceneUI::Draw()
{
	...
	for (int i = 0; i < *m_reserve_num_p; i++) {
		m_reserve_image_p->SetPos(m_ui_pos + CVector2D(i * 20 + 110, 55));
	...
	if (weapon_id != -1) {
		...
		case eWeaponSpear:
		case eWeaponAxe:
			m_weapon_image_p[weapon_id]->SetPos(m_ui_pos + CVector2D(185, 222.5));
			m_weapon_image_p[weapon_id]->SetSize(CVector2D(80, 80));
			m_weapon_image_p[weapon_id]->Draw();
			break;
		case eWeaponGun:
			m_weapon_image_p[weapon_id]->SetPos(m_ui_pos + CVector2D(187, 224.5));
