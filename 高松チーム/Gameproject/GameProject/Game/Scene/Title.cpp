...
m_rogo_pos(CVector2D(BACKXSIZE * 3 / 4 , BACKYSIZE *2 / 5)),
m_start_pos(CVector2D(BACKXSIZE * 3 / 4, BACKYSIZE * 2 / 3)),
m_ohuda_pos(CVector2D(BACKXSIZE * 3 / 4, BACKYSIZE * 2 / 5)),
m_cnt(0),
m_size(0.0f)
{
...
void Title::Draw()
{
    ...
    m_img_ohuda.SetPos(m_rogo_pos);
    /*static float time = 0;
    time -= 0.01f;
    m_img_ohuda.SetAng(time); */
    m_img_ohuda.Draw();
